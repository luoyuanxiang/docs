 ...
 */
function getItemsByDate(path: string): DefaultTheme.SidebarItem[] {
    // 侧边栏年份分组数组
    ...
    sync(`docs/${path}/*`, {
        ...
        // 添加年份分组
        yearGroups.unshift({
            text: `<img class="chinese-zodiac" style="position: static; vertical-align: middle; padding-bottom: 3px;" src="/svg/chinese-zodiac/${getChineseZodiac(Number(year.replace('年', '')))}.svg" title="${getChineseZodiacAlias(Number(year.replace('年', '')))}" alt="生肖">
 ...
 * @returns {DefaultTheme.SidebarItem[]}
 */
function getItems(path: string): DefaultTheme.SidebarItem[] {
    // 侧边栏分组数组
...
function addOrderNumber(groups: DefaultTheme.SidebarItem[]): void {
    for (let i: number = 0; i < groups.length; i++) {
        for (let j: number = 0; j < groups[i].items.length; j++) {
            const items: DefaultTheme.SidebarItem[] = groups[i].items;
            const index: number = j + 1;
            let indexStyle: string = `<div class="text-color-gray mr-[6px]" style="font-weight: 550; display: inline-block;">${index}</div>`;
            if (index == 1) {
